...
from keys.params import loadcerts
...
def log_request_details(p12_file, log_file, url, status_code, reason, cert_file, response_content):
    #TODO: Logging only accounts for minute, so more than one run per minute would have unexpected results
    try:
...
def generate_get_requests_from_spec(local_file_path, cert=None, retries=0, log_file=f"logs/requests{timestamp}.json", p12_file=""):
...
def cross_cert_test(local_file, retries=0):
    #TODO: Add in automated functionality to test across different certs at once
    logging.basicConfig(level=logging.INFO)
    logging.info(f"[*] Gathering API details.")
    try:
    ...
    for p12_file, p12_password in loadcerts.items():
        ...
        # TODO: Create a better logging solution than keeping all log entries in memory in a list
        # Write the updated list of logs back to the file
        log_file = f"logs/{p12_file}/{timestamp}.json"
        try:
            Path(f"logs/{p12_file}").mkdir(parents=True, exist_ok=True)
        except:
        ...
        for orig_path, methods in paths.items():
            if 'get' in methods:
                ...
                if not placeholders:
                    print(f"[*] Skipping path {orig_path} due to no required parameters.")
                    continue
            ...
            for placeholder in placeholders:
                if placeholder in params:
                    if isinstance(params[placeholder], list): #Check if testing multiple entries for single param
                        for value in params[placeholder]:
                            ...
                            try:
                                response = session.get(full_url)
                                print(f"[+] Response from {full_url}: {response.status_code} - {response.reason}")
                                #TODO: Fix logging for new function
                                log_request_details(p12_file, log_file, full_url, response.status_code, response.reason, p12_file, response.content.decode('utf-8'))
                            except Exception as e:
                                print(f"[!] Error performing GET request to {full_url}: {e}")
                                log_request_details(p12_file, log_file, full_url, response.status_code, response.reason, p12_file, "ERROR")
                    else:
                        path = path.replace(f'{{{placeholder}}}', params[placeholder])
                else:
                    missing_params.append(placeholder)
